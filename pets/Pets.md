...
Package package hu.progmasters.rex.pets;
...
hu └─ progmasters └─ rex └─ pets └─ Pet.java
...
Abstract Class public abstract class Pet
abstract means:
...
Dog extends Pet Cat extends Pet
...
Dog dog = new Dog(...); Cat cat = new Cat(...); 3. Fields (Attributes) private String name; private int age; private boolean cleansed;

Each pet has:

Field Type Meaning name String name of the pet age int age of the pet cleansed boolean whether the pet has been cleansed

Example object state:

name = "Rex" age = 3 cleansed = false 4. Constructor public Pet(String name, int age){ this.name = name; this.age = age; this.cleansed = false; }
...
name = Rex age = 3 cleansed = false 5. Getter Method public boolean isCleansed() { return cleansed; }

This method returns whether the pet is cleansed.

Example:

if(pet.isCleansed()){ // pet is clean }

The name isCleansed() follows Java convention for boolean getters.

Setter Method public void setCleansed(boolean cleansed) { this.cleansed = cleansed; }
...
Why Use an Abstract Parent Class?
Because dogs and cats share common features.

Instead of duplicating code:

Dog -> name, age, cleansed Cat -> name, age, cleansed
