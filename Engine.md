...
Parsing the command: refers to the process of analyzing a command line input—such as a user's typed command in a terminal or 
shell—to break it down into meaningful components like the command name, options (flags), and arguments. 
...
Keep running until Quit command appears
2.Read User Command
...
commandArray[0] = RegisterDog
commandArray[1] = Rex
commandArray[2] = 3
commandArray[3] = Male
commandArray[4] = Center1
...
RegisterAdoptionCenter | Center1
Register Dog
case "RegisterDog":
...
RegisterCat | Kitty | 2 | Female | Center1
Send Animal for Cleansing
...
Command is split
      ↓
Switch chooses action
      ↓
Method in RexInc is called
      ↓
Repeat until Quit
      ↓
Print final status
...
System status...
(adoption and animal information)
